...
describe("BStack demo test with Playwright", () => {
  ...
  test("login test", async () => {
    ...
    // Wait for and click the signin button
    await page.waitForSelector("#signin", { timeout: 10000 });
    await page.click("#signin");

    // Fill in username
    await page.waitForSelector("#username input", { timeout: 10000 });
    await page.fill("#username input", "locked_user");
    await page.keyboard.press('Enter');

    // Fill in password
    await page.fill("#password input", "testingisfun99");
    await page.keyboard.press('Enter');
    ...
    const errorText = await page.textContent(".api-error");
    
    expect(errorText === "Your account has been locked." || errorText === "Invalid Username").toBe(true);
  }, 60000);
